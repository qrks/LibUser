...
public class ServerOne {
	...
	public static void main(String[] args) {
		try {
			...
			ServerOne s = new ServerOne(System.out, 10);
	 ...
	 * connections.
	 **/
	public ServerOne(OutputStream logStream, int maxConnections) {
		setLogStream(logStream);
		log("Starting server");
		threadGroup = new ThreadGroup(ServerOne.class.getName());
	...
	public static class Control implements Service {
		ServerOne server; // The server we control
		 ...
		 **/
		public Control(ServerOne server, String password) {
			this.server = server;
